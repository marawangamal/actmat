...
class HFExpert:
    ...
    def make_stat_fetcher_map(self, layer_name):
        fetcher_map = {"covariance": self.get_layer_cov, "fisher": self.get_layer_fish}
        return lambda x: fetcher_map[x]
...
if __name__ == "__main__":
    ...
    expert_hf_dirs = [
        HFExpert(
            resolve(m),
            covariance_path=osp.join(
                args.expert_stats_dir, sanitize_hf_id(m), "covariance.pt"
            ),
            fisher_path=osp.join(args.expert_stats_dir, sanitize_hf_id(m), "fisher.pt"),
        )
    ...
    for layer_name in layer_to_file:
        ...
        if args.ignore_keep_pt and re.search(args.ignore_keep_pt, layer_name):
            ...
            if w_0.ndim != 2 or (
                ...
                merge_fn = getattr(merging, "merge_" + method)

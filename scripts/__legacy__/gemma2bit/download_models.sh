...
PRETRAINED_ID="google/gemma-2-2b-it"
FINETUNED_IDS=(
...
BASE="artifacts/checkpoints/gemma-2-2b-it"
...
if [[ -d "$PRETRAINED_DIR" ]]; then
  ...
  python scripts/gemma2bit/save_model_param_folder.py --model "$PRETRAINED_ID" --output-dir "$PRETRAINED_DIR"
fi
...
for hf_id in "${FINETUNED_IDS[@]}"; do
  ...
  if [[ -d "$ft_dir" ]]; then
    ...
    python scripts/gemma2bit/save_model_param_folder.py --model "$hf_id" --output-dir "$ft_dir"
  fi

...
Vendored from the Polyglot-Teachers paper (ljvmiranda921/polyglot-teachers,
scripts/lighteval_tasks.py). Requires the lighteval fork (./lighteval submodule,
0.13.1.dev0) for `MRewardBenchWeightedAccuracy`. The `mgsm_custom` task is kept
for completeness but is no longer run by the polyglot-all scripts — final M-GSM
comes from lm-eval `mgsm_native_cot` (see scripts/polyglot-all/eval_all-mgsm.sh).
"""

...
#   1. merge the 4 Polyglot OLMo3-7B experts (ar, cs, de, es)   [CPU, .venv-pg-mgsm]
#   2. eval the merged model on MMLU+MRB with the lighteval fork [GPU, .venv-pg-mmlu-mrb]
# M-GSM for the merges is scored separately by eval_all-mgsm.sh (lm-eval CoT).
#
# Usage:

...
Usage:
    python scripts/gemma2bit/merge.py \\
        --save artifacts/checkpoints/gemma-2-2b-it \\
        --merge-func sum \\
        --output-dir artifacts/checkpoints/gemma-2-2b-it/sum
"""
    ...
    for td in task_dirs:
        if not (td / "pretrained").exists() or not (td / "finetuned").exists():
            raise FileNotFoundError(
                ...
                "Run scripts/gemma2bit/download_models.sh first."
            )
...
if __name__ == "__main__":
    ...
    if args.save is None:
        args.save = "artifacts/checkpoints/gemma-2-2b-it"
    merge(args)

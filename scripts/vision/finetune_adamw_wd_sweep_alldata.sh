#!/bin/bash
#SBATCH --job-name=finetune_adamw_wd_alldata
...
# 3. AdamW WD sweep over the other 7 datasets x 3 wd values (Cars done separately).
#    lr pinned to 1e-5 by --optimizer=adamw. Index -> (dataset, wd):
#    dataset = DATASETS[id / 3], wd = WDS[id % 3]; saved per-wd so each wd
#    accumulates all 8 experts under checkpoints-adamw-wd{wd}/.
MODEL="ViT-B-16"
FT_MODE="standard"
OPTIMIZER="adamw"
...
echo "[BASH] AdamW WD sweep (alldata) | model: $MODEL | optimizer: $OPTIMIZER | wd: $WD | dataset: $DATASET | save dir: $SAVE_DIR"

...
SUBMOD_ORDER = {  # within-block ordering
    ...
    "self_attn.o_proj": 3, "mlp.gate_proj": 4, "mlp.up_proj": 5, "mlp.down_proj": 6,
}
ABBRV = {
    "model.layers.": "L", ".self_attn.": ".", ".mlp.": ".",
...
def main():
    rows = json.loads((REPO_ROOT / "artifacts/analysis/rm-loss/rm_loss_olmo.json").read_text())
    ...
    g = sns.relplot(
        df, x="layer_idx", y="metric", kind="line",
        hue="method", style="method", markers=True, aspect=4,
    )
    g.set(yscale="log")
    g.set_axis_labels("layer", "RegMean loss (vs. true covariance)")
    g.figure.suptitle("Olmo-3-7b — per-layer RegMean loss at each method's minimizer", y=1.03)
    ...
    out = REPO_ROOT / "artifacts/analysis/rm-loss/rm_loss_olmo.png"
    ...
    ax.set_yscale("log")
    ax.set_ylabel("total RegMean loss (vs. true covariance)")
    ax.set_title("Olmo-3-7b — total RegMean loss over all layers")

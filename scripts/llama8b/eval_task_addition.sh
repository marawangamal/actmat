...
# Merge + evaluate Llama-3.1-8B-Instruct (MergeBench) using EXACTLY MergeBench's
...
# tree at setup time — see scripts/gemma2bit/README.md (same patch applies here).
#
...
METHODS=(tsv actmat wudi mean isoc actmat_gd isoc2 isoc3 actmat_5k ace actmat_gd_5k)
...
if compgen -G "${OUT}/**/results_*.json" > /dev/null; then
  ...
  lm_eval --model hf --model_args "$MODEL_ARGS" \
    --tasks "$MULTILINGUAL_TASKS" --batch_size 16 --output_path "$OUT"
fi

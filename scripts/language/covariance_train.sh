#!/bin/bash
#SBATCH --job-name=t5base_cov_train
...
# One-off: collect train-set covariance for T5-base (FFT) at high nb and
# install as covariance.pt via symlink so the regmean eval consumes it.
set -euo pipefail
...
# fire. Target files (covariance-{train,test}-nb*-bs*.pt) are preserved.

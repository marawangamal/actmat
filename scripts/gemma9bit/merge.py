"""Merge Gemma-2-2B-IT MergeBench param-folder task vectors into a HuggingFace checkpoint.
...
Usage:
    python scripts/gemma9bit/merge.py \\
        --save artifacts/checkpoints/gemma-2-9b-it \\
        --merge-func sum \\
        --output-dir artifacts/checkpoints/gemma-2-9b-it/sum
"""
...
GEMMA_TASKS = ["instruction", "math", "coding", "multilingual"]
...
def merge(args):
    ...
    task_dirs = [save_root / t for t in GEMMA_TASKS]
    ...
    for td in task_dirs:
        if not (td / "pretrained").exists() or not (td / "finetuned").exists():
            raise FileNotFoundError(
                ...
                "Run scripts/gemma9bit/download_models.sh first."
            )

    pretrained_dir = (task_dirs[0] / "pretrained").resolve()
    print(f"Tasks          : {GEMMA_TASKS}")
...
if __name__ == "__main__":
    ...
    if args.save is None:
        args.save = "artifacts/checkpoints/gemma-2-9b-it"
    merge(args)
